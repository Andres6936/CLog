...
void assert_equal(std::string description, std::basic_string<CharT> expect, std::basic_string<CharT> result)
{
	...
	if (result != expect)
	{
		std::cout << "Test failed (" << typeid(CharT).name() << "): \"" << description << "\": \""
				  << cheaty_cast_string<char>(result) << "\" != \""
				  << cheaty_cast_string<char>(expect) << "\")\n";
		++failed;
	}
}
...
void test_int_bases()
{
	...
	test_format_it<CharT>("100100011010001010110011110001001101010111100110111101111", "{b}",
	...
	test_format_it<CharT>("1111111111111111111111111111111111111111111111111111111111010110", "{b}",
			int64_t(-42));
}
...
{
	test_format_it<CharT>("AAfooXX42YYbarBB", "AA{}XX{}YY{}BB", cheaty_cast_string<CharT>("foo"),
			42, cheaty_cast_string<CharT>("bar"));
}
...
int main()
{
	...
	std::cout << "Performed " << testcount << " tests, " << (testcount - failed) << " passed, "
